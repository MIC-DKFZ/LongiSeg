...
def resample_torch_simple_forautopet(
        data: Union[torch.Tensor, np.ndarray],
        new_shape: Union[Tuple[int, ...], List[int], np.ndarray],
        is_seg: bool = False,
        num_threads: int = 4,
        ...
        with torch.no_grad():
            ...
            if is_seg:
                if uniques is None:
                    ...
                    unique_values = torch.tensor(uniques)
                result_dtype = torch.int8 if max(unique_values) < 127 else torch.int16
                result = torch.zeros((data.shape[0], *new_shape), dtype=result_dtype, device=device)
                if not memefficient_seg_resampling:
                    # believe it or not, the implementation below is 3x as fast (at least on Liver CT and on CPU)
...
def resample_torch_forautopet(
    ...
    if do_separate_z:
        ...
        # reshape out of plane w/ nearest
        data = resample_torch_simple_forautopet(data, new_shape, is_seg=is_seg, num_threads=num_threads, device=device,
                                     memefficient_seg_resampling=memefficient_seg_resampling, mode=aniso_axis_mode,
                                     uniques=uniques)
        if was_numpy:
            data = data.numpy()
        ...
        return resample_torch_simple_forautopet(data, new_shape, is_seg, num_threads, device, memefficient_seg_resampling,
                                     uniques=uniques)